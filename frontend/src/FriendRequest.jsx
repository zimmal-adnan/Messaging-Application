...
function FriendRequest({ username, ws, setTargetUser, targetUser, }) {
    ...
    const [confirmRemove, setConfirmRemove] = useState(null); //stores the username of the friend that the user wants to remove
    ...
    function AcceptFriend(user) {
        ...
        //optimistic update
        setPendingRequests(pendingRequests.filter(u => u !== user)); //removing the friend from the pending list
        setFriends([...friends, user]); //adding the new friend to the friend list
    }
        ...
        <div className="friends-div">
            ...
            <div className="request-container">
                <input
                    ...
                    onKeyDown={(e) => e.key === "Enter" && setRequestInput()}
            ...
            <div>
                ...
                {pendingRequests.length === 0 ? ( //if no users in the list
                    ...
                    <ol>
                        ...
                        {pendingRequests.map(user => (
                            <li className = "friend-names" key={user}>
                                ...
                                <button className="friend-response" onClick={() => DeclineFriend(user)}><img src="x.jpg" className="img-response" alt="Decline"/></button>
                            </li>
                        ))}
                    </ol>
                )}
            ...
            {confirmRemove && ( //if confirmRemove is not null
                <div className="confirmation-box">
                    <div className="white-box">
                        <p>Are you sure you want to remove <strong>{confirmRemove}</strong> as a friend?</p>                    {/*clears the state to hide the confirmation box*/}
